...
const STRAPI = "https://api.khuta.om";
...
function _runServer(argv) {
  ...
  app.use("/saml/sso", async (req, res, next) => {
    ...
    try {
      ...
      const authCookie = rawHeaders[cookieIdx + 1];
      const splitCookie = authCookie.split("; ");
      const [khuta_auth] = splitCookie.filter((cookie) =>
        cookie.startsWith("khuta_auth")
      );
      ...
      const jwtToken = khuta_auth.split("=")[1];
      const userId = require("jwt-decode").jwtDecode(jwtToken)?.id;
      ...
      async function fetchAuthUser() {
        let response = await fetch(`${STRAPI}/api/users/${userId}`, {
          headers: {
